...
export default function App() {
    ...
    return (
        <main className="fixed top-0 bottom-0 left-0 right-0 flex justify-center items-center p-4 bg-gray-100">
            <div className="flex flex-col gap-4 bg-white p-4 rounded-xl w-full md:max-w-128 h-full md:max-h-96 shadow">
                <section className="grid grid-cols-[1fr_3rem] gap-2 w-full">
                    <input
                        type="text"
                ...
                <ul className="flex flex-col w-full h-full overflow-auto bg-white custom-scroll">
                    ...
                    {allTasks.map((task: Task) => (
                        <li key={"task-" + task.id} className="hover:bg-gray-100 grid grid-cols-[1.5rem_1fr_1.5rem] items-center justify-center p-4 gap-4 rounded-xl">
                            {task.completed ?
                            ...
                            <span className={"text-xl" + (task.completed ? " text-gray-500 line-through" : "")}>{task.text}</span>
